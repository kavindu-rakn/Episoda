...
export const AuthScreen: React.FC = () => {
  ...
  const [email, setEmail] = useState('johndoe789@gmail.com');
  ...
  const handleAuthSubmit = () => {
    ...
    if (isSignUp && !email.trim()) {
      hapticWarning();
      Alert.alert('Required Fields', 'Please enter your email address to sign up.');
      return;
    }
  ...
  return (
    <View style={[styles.container, { paddingTop: Math.max(insets.top, 16) }]}>
      ...
      >
        ...
        {/* Email Field (in Sign Up mode) */}
        {isSignUp && (
          <View style={styles.inputContainer}>
            <Feather name="mail" size={20} color={COLORS.darkGreen} style={styles.inputIcon} />
            <TextInput
              style={styles.input}
        ...
        <View style={styles.optionsRow}>
          ...
          >
            <View style={[styles.checkbox, rememberMe && styles.checkboxChecked]}>
              {rememberMe && <Feather name="check" size={14} color={COLORS.darkGreen} strokeWidth={3} />}
            </View>
          ...
          {!isSignUp && (
            <TouchableOpacity
              onPress={() => {
                hapticLight();
                Alert.alert('Reset Password', 'A password reset link would be sent to your email.');
              }}
              activeOpacity={0.7}
            >
